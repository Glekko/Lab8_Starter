describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    const buttonInnerText = await button.evaluate((el) => el.innerText);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of prodItems) {
      const shadowRoot = await item.evaluateHandle((item) => item.shadowRoot);
      const button = await shadowRoot.$('button');
      // Get the button's text
      const buttonText = await button.evaluate((el) => el.innerText);
      // Only click the button if the item hasn't been added to the cart yet
